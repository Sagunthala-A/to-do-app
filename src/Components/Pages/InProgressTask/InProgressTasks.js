...
const InProgressTasks = () => {
  const { taskItemsList, searchVal } = useContext(DataContext);
  let inProgressTaskArr = taskItemsList
    .filter((task) => !task.completed)
    .filter((val) => val.title.includes(searchVal));

  return (
    <div className="inProgressList__wrapper">
      <ul className="inProgressList">
        {inProgressTaskArr.length > 0 ? (
          inProgressTaskArr.map((task) => (
            <li className="inProgressList__list" key={task.id}>
              <h3>{task.title} </h3>
              <p>{task.description}</p>
              <p>Starting on: {task.createdOn}</p>
            </li>
          ))
        ) : (
          <h2>No Task is in progress !</h2>
